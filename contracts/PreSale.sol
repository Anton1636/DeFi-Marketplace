...
contract ICOMarketPplace{
  ...
  event TokenRecieved(address indexed token, address indexed from, uint amount);
  event TokenTransferred(address indexed token, address indexed from, uint amount);
  event TokenWithraw(address indexed token, address indexed from, uint amount);
  ...
  modifier onlyCreator(address _token){
    require(msg.sender == tokenDetails[_token].creator, "Caller is not the token cerator");
    _;
  }

  receive() external payable{
    revert("Contract doen't accept ETH directly");
  }
  ...
  function creteICOSale(address _token, uint256 _price) external {
  ...
  function buyToken(address _token, uint256 _amount)external payable supportedToken(_token){
    ...
    require(sent, "Failed to transfer ETH to token creator");
  ...
  function withwrawToken(address _token, uint256 _amount) external onlyCreator(_token) supportedToken(_token){
    require(_amount > 0, "AMount must be created greater then 0");
    ...
    emit TokenWithraw(_token, msg.sender, _amount);
  }
  ...
  function getTokenCreateBy(address _creator) external view returns(TokenDetails[] memory){
    uint256 count = 0;
